...
面向农业巡检、工业运维等多目标作业场景，构建**上层 TSP 序列规划 + 下层 AirSim 强化学习导航避障**的两层系统：上层用指针网络快速近似最优访问顺序，下层用 SAC 等 DRL 策略实现连续空间避障飞行；**中期之后**在 TSP 序列训练中引入障碍物惩罚，使上层规划与下层可飞性对齐，并通过联合评测验证“路径质量—任务成功率”的协同关系。
...
**核心科学问题**：上层优化的是无障碍欧氏路径质量，下层执行受静态障碍约束；二者目标不一致时，如何通过障碍物感知 TSP 与联合评测，提升整链任务成功率并保持可接受的路径代价。
...
- 论证主线：联合评测揭示“路径质量—任务成功率”协同/冲突，为障碍物感知 TSP 提供动机
...
**解读**：联合评测仅对比 `dataset_500.npy` 中两组对齐序列；几何最优序列在含障环境中任务成功率低于 RL 规划序列（67% vs 82%），初步验证“路径质量—任务成功率”并非单调一致，为中期之后引入障碍物惩罚 TSP 提供依据。
